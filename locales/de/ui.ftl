...
command-name-install-bsdtar = bsdtar installieren (libarchive-tools)
command-name-install-makepkg = makepkg installieren (pacman)
...
dep-wizard-bsdtar-desc = bsdtar (Teil von libarchive-tools) wird von makepkg benötigt, um Quellarchive beim Erstellen von AUR-Paketen zu entpacken.
dep-wizard-makepkg-desc = makepkg (Teil des pacman-Pakets) wird benötigt, um .pkg.tar.zst-Pakete aus einem PKGBUILD für Arch Linux zu erstellen.
