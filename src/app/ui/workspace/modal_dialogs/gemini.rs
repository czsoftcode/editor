...
pub fn show(
    ...
    StandardModal::new(i18n.get("gemini-title"), "gemini_modal")
        ...
        .show(
            ...
            |ui| {
                // --- TLAČÍTKA V PATIČCE (zprava doleva) ---
                if ui.button(i18n.get("btn-close")).clicked() {
                    action = Some(GeminiModalAction::Close);
                }

                if response_text.is_some() && ui.button(i18n.get("gemini-btn-new")).clicked() {
                    action = Some(GeminiModalAction::NewQuery);
                }

                let can_send = !loading;
                if ui.add_enabled(can_send, egui::Button::new(i18n.get("gemini-btn-send"))
                    .fill(egui::Color32::from_rgb(40, 80, 150))).clicked()
                {
            ...
            GeminiModalAction::Send => {
                if !ws.gemini_prompt.trim().is_empty() {
                    ...
                    let active_path = ws.editor.active_path().map(|p| {
                        p.strip_prefix(&ws.root_path).unwrap_or(p).to_string_lossy().into_owned()
                    });
                    let active_content = ws.editor.active_tab
