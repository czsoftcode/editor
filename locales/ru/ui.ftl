...
command-name-install-bsdtar = Установить bsdtar (libarchive-tools)
command-name-install-makepkg = Установить makepkg (pacman)
...
dep-wizard-bsdtar-desc = bsdtar (часть libarchive-tools) требуется makepkg для распаковки исходных архивов при сборке AUR-пакетов.
dep-wizard-makepkg-desc = makepkg (часть пакета pacman) требуется для сборки .pkg.tar.zst пакетов из PKGBUILD для Arch Linux.
