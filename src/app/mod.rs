...
impl EditorApp {
    ...
    fn setup_custom_fonts(ctx: &egui::Context) {
        ...
        let preferred_mono = ["jetbrainsmono", "notomono", "dejavusansmono", "ubuntumono", "liberationmono"];
        let preferred_prop = ["ubunturegular", "notosansregular", "dejavusans", "notosans", "ubuntu", "roboto", "liberationsans"];
        let preferred_symbols = ["symbola", "nerdfont", "notocoloremoji", "notoemoji", "symbol"];
        ...
        for dir in search_dirs {
            ...
            for entry in walkdir::WalkDir::new(dir).follow_links(true).into_iter().flatten() {
                ...
                if loaded_primary_mono.is_some() && loaded_primary_prop.is_some() && loaded_symbols.len() > 10 {
                    break;
                }
            }
            if loaded_primary_mono.is_some() && loaded_primary_prop.is_some() && !loaded_symbols.is_empty() { break; }
        }
