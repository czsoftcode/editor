...
command-name-install-bsdtar = Install bsdtar (libarchive-tools)
command-name-install-makepkg = Install makepkg (pacman)
...
dep-wizard-bsdtar-desc = bsdtar (part of libarchive-tools) is required by makepkg to extract source archives when building AUR packages.
dep-wizard-makepkg-desc = makepkg (part of the pacman package) is required to build .pkg.tar.zst packages from a PKGBUILD for Arch Linux.
