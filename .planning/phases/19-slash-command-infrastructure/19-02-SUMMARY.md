...
plan: 02
subsystem: ui
tags: [slash-commands, async, mpsc, background-polling, ai-chat]

# Dependency graph
requires:
  - phase: 19-slash-command-infrastructure
    provides: "slash dispatch infrastructure, SlashResult enum, SYSTEM_MSG_MARKER, command registry"
provides:
...
affects: [20-gsd-port-frontmatter-parser, 21-gsd-port-state-operations]

# Tech tracking
tech-stack:
  added: []
  patterns: [async-slash-commands-via-mpsc, in-place-conversation-update, stale-result-gen-counter]
...
patterns-established:
  - "Async slash command pattern: spawn thread -> store receiver in ws -> poll in background.rs -> update conversation in-place"
  - "Stale result detection: generation counter incremented on /clear and /new"

requirements-completed: [SLASH-04, SLASH-05, SLASH-06]
