...
command-name-install-aur = Instalovat cargo-aur
command-name-install-bsdtar = Instalovat bsdtar (libarchive-tools)
command-name-install-makepkg = Instalovat makepkg (pacman)
...
command-name-install-tar = Instalovat tar
...
dep-wizard-aur-desc = Nástroj cargo-aur je vyžadován pro vytváření PKGBUILD souborů a AUR balíčků pro Arch Linux.
dep-wizard-bsdtar-desc = bsdtar (součást libarchive-tools) je vyžadován nástrojem makepkg pro rozbalování zdrojových archivů při sestavování AUR balíčků.
dep-wizard-makepkg-desc = makepkg (součást balíčku pacman) je vyžadován pro sestavení .pkg.tar.zst balíčků z PKGBUILD pro Arch Linux.
...
dep-wizard-tar-desc = Nástroj tar je nezbytný pro vytváření komprimovaných archivů .tar.gz pro Linux a BSD systémy.
