...
- "M005 decomposed into 2 slices: S01 (search engine + togglery + file filtr + UI dialog se zvýrazněnými výsledky) a S02 (project-wide replace s preview + local history) — S01 high-risk kvůli egui LayoutJob klikatelnost + kontext sloučení, S02 medium-risk kvůli LocalHistory borrow constraints v replace flow"
- "File type filtr sloučen do S01 (ne separátní slice) — glob match na PathBuf v search threadu je triviální, nepotřebuje vlastní vertikální slice"
- "Regex capture groups ($1, $2) v replace — automaticky přes Regex::replace_all(), žádný explicitní UX/validace — scope by se zbytečně rozšířil"
- "Replace error handling: per-file, ne atomic all-or-nothing — pokud snapshot selže, skip soubor; pokud write selže, toast a pokračovat s dalšími"
- "Toggle buttons jako selectable_label s textem '.*' / 'Aa' / 'W' (VS Code styl) — nejjednodušší egui pattern bez custom widgetů"
- "Kontextové řádky ±2 se sloučením blízkých matchů (distance ≤ 2*context) — grep -C styl, vizuálně čistší než opakující se řádky"
- "SearchBatch enum (Results/Done/Error) pro streamování místo single-shot Vec<SearchResult> — per-soubor dávkování pro inkrementální UI"
- "Replace preview: collapsible per-file sekce s checkboxy a diff view — pattern z diff_view.rs"
- "take_snapshot() volaný v workspace handleru (main thread), ne v background threadu — LocalHistory není Send, &mut borrow potřebný"
- "build_regex() jako centrální factory pro regex z query + SearchOptions — plain mode escapuje, regex mode přímé Regex::new, whole-word obalí \b, case přes RegexBuilder.case_insensitive(). Error vrací popisný String, ne panic."
- "SearchBatch enum (Results/Done/Error) jako typed protokol přes mpsc — per-soubor dávkování umožňuje inkrementální UI akumulaci. Done/Error uzavírají kanál logicky."
- "search_file_with_context() jako pure funkce (path+regex+context_lines → Vec<SearchResult>) — sloučení blízkých matchů (distance ≤ 2*context_lines) do jednoho kontextového bloku, ale každý match generuje vlastní SearchResult"
- "File filter glob matchuje jak filename tak celou relativní cestu — umožňuje i prefix/cesta filtry (e.g. 'src/**/*.rs')"
- "LayoutJob s multi-section pro search match highlighting: prefix (šedý line number) + normální text + zvýrazněný match (oranžový background rgba(200,130,0,120)). Kontextové řádky dim barvou rgb(140,140,140)."
...
- "M006 decomposed into 2 slices: S01 (inline search panel — layout, input, výsledky, klik→jump, replace, i18n) a S02 (in-file search regex togglery) — S01 high-risk kvůli egui layout pořadí a fokus transfer, S02 low-risk izolovaná změna v editor/search.rs"
- "show_panel nahrazuje show_input jako primární visibility flag pro project search — show_input se smaže po přepojení keymapu a menubar"
- "Replace preview dialog zůstává modální i v M006 — vhodný pro potvrzovací flow, inline replace by zbytečně komplikoval panel"
- "In-file search toggle stav (regex/case/whole-word) persistuje v Editor struct mezi otevřeními Ctrl+F — fieldy přetrvávají přirozeně, nesdílejí se s project search (nezávislé kontexty)"
...
- "M007 jako single-slice milestone — modal, guard rozšíření, reinicializace a i18n tvoří jeden koherentní vertikální flow, rozdělit by znamenalo nedemoovatelné mezikroky"
- "pending_open_choice: Option<PathBuf> na WorkspaceState — centrální stav pro pending modal volby, přepisuje se při novém výběru (žádná akumulace)"
- "AppAction::OpenWithChoice(PathBuf) pro wizard callback — wizard nemá &mut WorkspaceState v callbacku, potřebuje cestu přes AppAction na workspace úroveň"
